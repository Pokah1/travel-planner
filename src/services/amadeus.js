...
export async function searchDestinations(keyword) {
  ...
  const response = await fetch(
    `${BASE_URL}/v1/reference-data/locations?subType=CITY&keyword=${encodeURIComponent(keyword)}&page%5Blimit%5D=10`,
    { headers: { Authorization: `Bearer ${token}` } }
  )

  if (!response.ok) {
    const text = await response.text()
    throw new Error(`Destination search failed: ${response.status} - ${text}`)
  }
