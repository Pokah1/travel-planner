...
useEffect(() => {
      ...
      <section>
        ...
        {hotels.length ? (
          <ul className="grid gap-4 md:grid-cols-2">
            {hotels.map((hotel) => (
             <li key={hotel.id || hotel.name} className="p-4 border rounded-lg relative">
  ...
  <ul className="space-y-3">
    {flights.map((flight) => {
      ...
      return (
        <li key={flight.id || `${segment?.departure?.iataCode}-${segment?.arrival?.iataCode}-${flight.price?.total}`} className="p-4 border rounded-lg">
          {segment ? (
            <>
